...
_non_uppercase_re = re.compile(
    r"\b({})\b".format("|".join(non_uppercase_exceptions.keys())), re.IGNORECASE
)
...
def spell_acronyms(text):
    text = re.sub(
        _non_uppercase_re, lambda m: non_uppercase_exceptions[m.group(0).lower()], text
    )
