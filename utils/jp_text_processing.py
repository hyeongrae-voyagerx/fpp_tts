...
romaji_dict = {".": "ten", "0": "zero", "1": "ichi", "2": "ni", "3": "san", "4": "yon", "5": "go", "6": "roku",
               "7": "nana",
...
def convert(convert_num, dict_choice):
    ...
    convert_num = unicodedata.normalize('NFKC', convert_num)
    ...
    while convert_num[0] == "0" and len(convert_num) > 1 and convert_num:
        convert_num = convert_num[1:]
...
def convert_kanji(convert_num: float):
...
def jp_num2kanji(text):
    phone_pattern = r'\(?\d{2,3}\)?[-.\s]?\d{3,4}[-.\s]\d{3,4}(?:[-.\s]\d{3,4})?'
    phone_numbers = re.findall(phone_pattern, text)
    for phone_number in phone_numbers:
        phone_number_kanji = []
        sections = re.split(r'[-.\s]', phone_number)
        for section in sections:
            section_kanji = [convert(num, "kanji") for num in section]
            phone_number_kanji.append("".join(section_kanji))
        phone_number_kanji = ", ".join(phone_number_kanji)
        text = text.replace(phone_number, phone_number_kanji, 1)

    leading_zero_pattern = r'(?<![.,\d])0+\d+'
    leading_zeros = re.findall(leading_zero_pattern, text)
    for leading_zero in leading_zeros:
        raw_number = re.findall(r'\d', leading_zero)
        text = text.replace(leading_zero, " ".join(raw_number), 1)

    numbers = re.findall(r'\d{1,3}(?:,\d{3})*\.?\d*', text)
    for number in numbers:
        number = number.replace(",", "")
        number_text = convert(number, "kanji")
        text = text.replace(number, number_text, 1)

    return text

def jp_punc2punc(text):
    # Mapping of full-width Japanese punctuation to half-width English equivalents.
    punctuation_mapping = {
        '、': ',',  # Japanese comma to English comma
        '。': '.',  # Japanese period to English period
        '「': '"',  # Japanese opening quote to English double quote
        '」': '"',  # Japanese closing quote to English double quote
        '（': '(',  # Japanese opening parenthesis to English opening parenthesis
        '）': ')',  # Japanese closing parenthesis to English closing parenthesis
        '・': '/',  # Japanese dot to slash (commonly used in dates)
        '！': '!',  # Japanese exclamation mark to English exclamation mark
        '？': '?',  # Japanese question mark to English question mark
        # Add more mappings as needed
    }

    # Replace each full-width punctuation with its half-width counterpart
    for jp, en in punctuation_mapping.items():
