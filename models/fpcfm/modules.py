...
from ..modules.style_encoder import StyleAdaptiveLayerNorm
...
class AdaLayerNormZero(nn.Module):
    ...
    def forward(self, x, emb = None):
        ...
        shift_msa, scale_msa, gate_msa, gate_mlp = torch.chunk(emb, 4, dim=1)

        x = self.norm(x) * (1 + scale_msa[:, None]) + shift_msa[:, None]
        return x, gate_msa, gate_mlp
...
class DiTBlock(nn.Module):
    ...
    def __init__(self, dim, heads, dim_head, ff_mult = 4, dropout = 0.1):
        ...
        self.ff_norm_saln = StyleAdaptiveLayerNorm(dim, 128)
        self.ff = FeedForward(dim = dim, mult = ff_mult, dropout = dropout, approximate = "tanh")

    def forward(self, x, t, style_vector, drop_saln, mask = None, rope = None): # x: noised input, t: time embedding
        # pre-norm & modulation for attention input
        norm, gate_msa, gate_mlp = self.attn_norm(x, emb=t)
        ...
        if drop_saln:
            norm = self.ff_norm_saln(x, style_vector)
        else:
            norm = self.ff_norm_saln(x, torch.zeros_like(style_vector))
