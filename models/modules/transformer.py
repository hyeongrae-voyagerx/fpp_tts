...
from .style_encoder import StyleAdaptiveLayerNorm
...
class TransformerLayer(nn.Module):
    def __init__(
        ...
        self.pos_ff = PositionwiseConvFF(
            d_model, d_inner, kernel_size, dropout, pre_lnorm=kwargs.get("pre_lnorm")
        )
...
class TransformerLayerWithSALN(nn.Module):
    def __init__(
        self, n_head, d_model, d_head, d_inner, kernel_size, dropout, **kwargs
    ):
        super(TransformerLayerWithSALN, self).__init__()

        self.dec_attn = MultiHeadAttn(n_head, d_model, d_head, dropout, **kwargs)
        self.pos_ff = PositionwiseConvFF(
            d_model, d_inner, kernel_size, dropout, pre_lnorm=kwargs.get("pre_lnorm")
        )
        self.saln_0 = StyleAdaptiveLayerNorm(d_model, 128)  # d_model, style_dim
        self.saln_1 = StyleAdaptiveLayerNorm(d_model, 128)  # d_model, style_dim

    def forward(self, dec_inp, style_vector, mask=None):
        output = self.dec_attn(dec_inp, attn_mask=~mask.squeeze(2))
        output = self.saln_0(output, style_vector)
        output *= mask
        output = self.pos_ff(output)
...
class FFTransformer(nn.Module):
    ...
    def forward(self, dec_inp, seq_lens=None, conditioning=0):
        if self.word_emb is None:
            inp = dec_inp
            mask = mask_from_lens(seq_lens, max_len=dec_inp.size(1)).unsqueeze(2)
        else:
        ...
        if self.saln:
            ...
            for layer in self.layers:
                out = layer(out, style_vector=conditioning, mask=mask)
        else:
